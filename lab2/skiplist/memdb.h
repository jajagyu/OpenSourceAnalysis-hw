...
struct MemDBOptions {
  // mutable memtable이 이 크기를 넘기면 immutable로 회전한다.
...
class InMemoryDB {
 ...
 private:
  struct MemTable {
    ...
    // 하나의 skiplist가 하나의 memtable 데이터 컨테이너 역할을 한다.
    SkipList list;
    // 추정 메모리 사용량(간단 모델).
    size_t size_bytes;
  ...
  // 새 엔트리 기록 전, mutable 용량이 충분한지 확인하고 필요 시 회전한다.
